...
### 15.3 ⭐ Sweet Spot 5 問體檢（v2.2.1 統一 0-10 量表）

> 為對齊 write-prd-v2 v2.2.1 統一商業化公式，將 §15.1 各 Q 評分加總，並明確計算 sweet_score。

| # | 甜蜜點問題 | 評分 (0-10) | 對應 §15.1 章節 |
|---|---|---|---|
| **Q1** | 目標市場是否真實存在且可觸達？ | **3** | 當沖族 + 求職族縮小範圍後**勉強成立**（PTT 流量年減 12%，目標族群只在 2 個板） |
| **Q2** | 既有方案是否真的不足？ | **4** | RSS / Discord bot 不支援中文 + LINE 推爆警示 + AI 摘要，**有差異化但非護城河** |
| **Q3** | 付費意願是否真實？ | **4** | NT$99/月對當沖族可接受（vs 損失一筆交易 NT$5,000），但** Freemium 轉付費率 < 5% 待驗證** |
| **Q4** | 是否有結構性護城河？ | **2** | 爬蟲 + GPT-4o-mini 皆 commodity，**無結構護城河**，只能靠用戶累積 |
| **Q5** | Sean 一人公司是否可 scale？ | **5** | 技術棧簡單，**但客服成本高、PTT 政策變動風險大** |
| **加總 (sweet_score)** | — | **18/50 = 3.6/10** | 🟡 **PIVOT 等級**（3-5 = 縮小範圍 + Pivot） |
...
**最終商業化分數**：**52 / 100**（自原 48 微調 — sweet_score 從 3 提升到 3.6，PTT 雖下滑但仍有當沖+求職甜蜜點）

**行動建議**：🟡 **PIVOT + SMALL SCOPE**
- ✅ **不做**：企業版 / 多語系 / Dcard / Threads（超出範圍）
- ✅ **做**：當沖族 + 求職族 2 板 5 關鍵字 + AI 摘要 + LINE 推播（最小可行）
- ⚠️ **Pivot 觸發**：若 6 個月付費 < 100 人 → 轉 Dcard 監控或撤退
- ⚠️ **PTT 風險**：流量年減 12%，3 年內可能邊緣化，需有 Threads/Bluesky 備案

### 15.4 ⭐ 競爭對手 Sweet Spot 對照表（v2.2.1 新增）

| 對手 | 甜蜜點 | 我們甜蜜點 | 交集 |
|---|---|---|---|
| **RSS 訂閱** | 免費 + 多源整合 | AI 摘要 + LINE 推爆警示 | 極小（重度用戶才願付費） |
| **Discord bot（如 Ptt-Dicord-Bot）** | 免費 + 多語言社群 | 中文 AI 摘要 + 個人化關鍵字 | **當沖族**（Discord 偏年輕、非當沖族主力） |
| **Ptt-Alertor 開源版** | 免費 + 開源 + 支援多板 | 付費 + AI + LINE | **求職族**（願付費省時間） |
| **Threads / Dcard 監控** | 平台原生體驗 | PTT 特定板 | 無（不同平台） |

**甜蜜點結論**：PTT 當沖 + 求職 2 板的「**AI 摘要 + LINE 推爆警示**」是唯一可嘗試的細分，但市場規模有限（≤ 5,000 付費用戶天花板），**不建議主力投入**。
